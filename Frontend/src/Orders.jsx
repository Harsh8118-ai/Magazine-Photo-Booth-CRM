...
export default function Orders() {
    ...
    const handleChange = (e) => {
        ...
        if (name === "bookingAmount" || name === "tokenAmount") {
            const booking = parseFloat(updatedForm.bookingAmount) || 0;
            const token = parseFloat(updatedForm.tokenAmount) || 0;
            updatedForm.remainingAmount = booking - token;
        }
    ...
    const addClient = async () => {
        ...
        if (!form.bookingAmount || !form.tokenAmount) return alert("Booking and Token required");
        ...
        try {
            ...
            setForm({
                name: "", eventType: "", eventDate: "", eventLocation: "", phone: "", bookingAmount: "",
                tokenAmount: "",
                remainingAmount: ""
            });
            console.log(form);
            ...
            <Card className="bg-gray-200 border border-gray-300">
                <CardContent>
                    <div className="grid grid-cols-1 sm:grid-cols-6 gap-4 items-center">
                        ...
                        <Input type="number" placeholder="Phone Number *" name="phone" value={form.phone} onChange={handleChange} />
                        <Input type="number" placeholder="Booking Amount *" name="bookingAmount" value={form.bookingAmount} onChange={handleChange} />
                        <Input type="number" placeholder="Token Amount *" name="tokenAmount" value={form.tokenAmount} onChange={handleChange} />
                            ...
                            {filteredClients.map((c) => {
                                ...
                                return (
                                    <tr key={c._id} className={`${rowClass} hover:bg-gray-100`}>
                                        ...
                                        <td className="p-2">₹{c.bookingAmount}</td>
                                        <td className="p-2">₹{c.tokenAmount}</td>
                                        <td className="p-2">₹{c.remainingAmount}</td>
